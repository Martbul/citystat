...
import 'package:citystat_infra/colorscheme/colorscheme.dart';
import 'package:citystat_infra/plugins/service/models/exceptions.dart';
import 'package:citystat_infra/theme.dart';
...
class FlowyDynamicPlugin {
  ...
  /// The plugins should be loaded into a folder with the extension `.citystat_plugin`.
  ...
  static const String ext = 'citystat_plugin';
  ...
  ///
  /// If the plugin loaded does not contain the `.citystat_plugin` extension, this
  ...
  static Future<FlowyDynamicPlugin> decode({required Directory src}) async {
    ...
    if (!isPlugin(src)) {
      throw PluginCompilationException(
        'The plugin directory must have the extension `.citystat_plugin`.',
      );
    }
  ...
  /// > directory.citystat_plugin // plugin root
  ...
  static Future<FlowyDynamicPlugin> _theme({required Directory src}) async {
    ...
    try {
      ...
      throw PluginCompilationException(
        'The theme plugin does not adhere to the following format: `<plugin_name>.citystat_plugin`.',
      );
    }
